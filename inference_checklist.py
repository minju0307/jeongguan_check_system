...
def inference(input_text, model_path):
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        model_path, num_labels=len(id2label), id2label=id2label, label2id=label2id, problem_type="multi_label_classification"
    )

    device = "cuda"
    model.to(device)
    encoding = tokenizer(input_text, return_tensors="pt", truncation=True, max_length=512)
    ...
    predicted_questions = [id2label[idx] for idx, label in enumerate(predictions) if label == 1.0]
