...
class TestLLM(unittest.TestCase):
    ...
    def test_llm_answer(self):
        ...
        answer = generate_answer(self.gpt_model, "\n".join(paragraphs), question)
    ...
    def test_llm_answer_langchain(self):
        ...
        prompt = ChatPromptTemplate.from_template("""정관 문서: {paragraph}
질문: {question}
===
...
"""
        ...
        prompt = ChatPromptTemplate.from_template("""[회사의 정관 검토 내역]
Q.감사의 임기는 얼마인가? A.2년
...
""")
    ...
    def test_llm_advice_and_reference(self):
        question = '정기주주총회를 개최하는가?'
        answer = '예, 정기주주총회는 매 결산기가 끝난 후 3월 내에 소집됩니다.'
        sangbub = """제365조(총회의 소집)
                ①정기총회는 매년 1회 일정한 시기에 이를 소집하여야 한다.
