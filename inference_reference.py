...
def retrieval_query(tokenizer, model, query, index, top_k, device):
    ...
    search_index = list(index.search(np.float32(query_embedding.detach().cpu()), int(top_k))[1][0])
...
def main(top_k, query):
    ...
    query, references = retrieval_query(tokenizer, model, query, index, config_dict["top_k"], device)
